...
* BN：在C，H，W上作归一化，使每一个batch分布稳定为0，1的标准分布，降低模型学习难度，避免梯度消失、梯度爆炸。全连接层或卷积操作之后，激活函数之前。![1706151503881](image/自然语言处理基础/1706151503881.png)加入缩放和平移变量y，b的原因是：保证每一次数据经过归一化后还保留原有学习来的特征，同时又能完成归一化操作，加速训练。 这两个参数是用来学习的参数。不适用于深度不固定的网络。
* LN 对每个样本的 C、H、W 维度上的数据求均值和标准差。
* ![1706151671967](image/自然语言处理基础/1706151671967.png)无需批训练，可以处理batch=1的情况。LN用于RNN效果比较明显，但是在CNN上，效果不如BN。
* IN：IN针对图像像素做normalization，最初用于图像的风格化迁移。不能将不同channel的像素一起平均，不利于风格化，因此对单独channel进行归一化。
* GN：GN是为了解决BN对较小的mini-batch size效果差的问题。GN适用于占用显存比较大的任务。计算公式类似LN，但c变为c/g。
