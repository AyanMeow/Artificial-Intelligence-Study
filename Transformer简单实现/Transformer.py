...
class TransformModel(object):
    ...
    def shifft_right(self,tokens):
        #在句子开头添加<bos>
        #如何做到？
        shiffted_token=['<bos>']+tokens
        return shiffted_token
    
    def transformer(self,inputs):
        X=self.word_emb(inputs)
        C=self.encoder_stack(X)
        SX=self.shifft_right(X)
        P=self.decoder_stack(SX,C)
        O=layers.Dense(self.vocab_size)(P)
